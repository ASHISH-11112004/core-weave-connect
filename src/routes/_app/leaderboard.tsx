      ...
      <div className="grid sm:grid-cols-3 gap-4">
        {top3.map((e, i) => {
          ...
          return (
            ...
            >
              <Surface
                className={`p-6 text-center relative overflow-hidden ${i === 0 ? "sm:scale-105" : ""}`}
              >
                <div
                  className={`absolute -top-20 left-1/2 -translate-x-1/2 h-40 w-40 rounded-full bg-gradient-to-br ${colors[i]} opacity-20 blur-3xl`}
                />
                <div className="relative">
                  <div
                    className={`mx-auto h-14 w-14 rounded-2xl bg-gradient-to-br ${colors[i]} grid place-items-center text-white shadow-lg`}
                  >
          ...
          <table className="w-full text-sm min-w-[420px]">
            <thead className="bg-muted/40 text-muted-foreground text-xs">
              <tr>
                ...
                <th className="text-left py-3 hidden sm:table-cell">Department</th>
                ...
                <tr key={e.id} className="border-t border-border hover:bg-muted/30 transition">
                  ...
                  <td className="py-3">
                    <div className="flex items-center gap-3">
                      <Avatar initials={e.avatar} className="h-8 w-8 text-xs" />
                      <span>{e.name}</span>
                    </div>
                  </td>
