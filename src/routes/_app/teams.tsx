              ...
              <Surface hover className="p-5">
                ...
                <div className="mt-4 flex -space-x-2">
                  {members.map((m) => (
                    <div
                      key={m.id}
                      className="h-8 w-8 rounded-full gradient-accent text-white grid place-items-center text-[10px] font-semibold ring-2 ring-card"
                    >
                      {m.avatar}
                    </div>
                  ))}
                  {d.count > members.length && (
                    <div className="h-8 w-8 rounded-full bg-muted text-muted-foreground grid place-items-center text-[10px] ring-2 ring-card">
                      +{d.count - members.length}
                    </div>
                  )}
                </div>
