      ...
      <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-end sm:justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            className="inline-flex items-center gap-1.5 h-9 px-3 sm:px-3.5 rounded-lg bg-card border border-border text-[12.5px] font-medium hover:bg-muted transition"
          >
      ...
      <Surface elevated className="relative overflow-hidden p-5 sm:p-7 md:p-9">
        ...
        <div className="relative flex flex-col lg:flex-row lg:items-center gap-6 sm:gap-8 justify-between">
          <div className="max-w-2xl">
            <Badge color="purple">
              <Sparkles className="h-3 w-3" /> Leadership message
            </Badge>
          ...
          <div className="grid grid-cols-2 gap-2.5 sm:gap-3 w-full lg:w-auto lg:min-w-[300px]">
            {[
              ...
              <div key={s.label} className="frost rounded-2xl border border-border/80 p-3 sm:p-4">
                ...
                <div className="mt-2.5 sm:mt-3 text-[20px] sm:text-[24px] font-semibold tabular tracking-tight">
                  {s.value}
                </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {kpis.map((k, i) => (
          <Surface key={k.label} delay={i * 0.04} hover className="p-4 sm:p-5">
            ...
            <div className="mt-2.5 text-[22px] sm:text-[26px] md:text-[28px] font-semibold tracking-tight tabular leading-none">
              {k.value}
            </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4 sm:gap-5">
        <Surface className="lg:col-span-2 p-4 sm:p-6" delay={0.05}>
          <SectionTitle
            icon={Megaphone}
            action={
              <button className="text-[11.5px] text-muted-foreground hover:text-primary transition">
                View all
              </button>
            }
          >
            ...
            {announcements.slice(0, 3).map((a, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="font-medium text-[13.5px] mt-1 group-hover:text-primary transition">
                    {a.title}
                  </p>
                  <p className="text-[12.5px] text-muted-foreground line-clamp-1 mt-0.5">
                    {a.body}
                  </p>
                </div>
              </motion.div>
          ...
          <ul className="space-y-3.5">
            {events.slice(0, 4).map((e) => (
              <li key={e.id} className="flex gap-3 group cursor-pointer">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[12.5px] font-medium truncate group-hover:text-primary transition">
                    {e.title}
                  </p>
                  <p className="text-[11px] text-muted-foreground tabular">
                    {e.time} · {e.attending} attending
                  </p>
                </div>
              </li>
            ))}
              ...
              <li key={r.id} className="flex gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-[12.5px]">
                    ...
                    <Badge color="purple">{r.badge}</Badge>
                  </p>
          ...
          <ul className="space-y-3.5">
            {discussions.slice(0, 4).map((d) => (
              <li key={d.id} className="flex gap-3 cursor-pointer group">
                <div className="h-8 w-8 rounded-lg bg-muted grid place-items-center text-[9.5px] font-semibold text-muted-foreground">
                  #{d.channel.slice(0, 2)}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[12.5px] font-medium truncate group-hover:text-primary transition">
                  ...
                  <p className="text-[11px] text-muted-foreground tabular">
                    {d.author} · {d.replies} replies {d.hot && "· 🔥"}
                  </p>
                </div>
              </li>
            ))}
        ...
        <Surface className="p-4 sm:p-6 md:col-span-2 lg:col-span-1" delay={0.15}>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {[
              ...
              { label: "Post update", desc: "Share news", action: () => setIsNewPostOpen(true) },
