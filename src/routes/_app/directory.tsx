...
function Page() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Employee Directory" subtitle={`${employees.length} teammates · ${departments.length} departments`} />

      <Surface className="p-3 flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[220px]">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search name, role, skill…"
            ...
            <Surface hover className="p-5">
              <div className="flex items-start gap-3">
                <div className="relative">
                  ...
                  <span className="absolute -bottom-0.5 -right-0.5"><StatusDot status={e.status} /></span>
                </div>
              ...
              <div className="mt-3 flex flex-wrap gap-1">
                {e.skills.slice(0, 3).map((s) => <span key={s} className="text-[10px] bg-muted px-2 py-0.5 rounded-full text-muted-foreground">{s}</span>)}
              </div>
              <div className="mt-4 pt-4 border-t border-border flex gap-2">
                <button className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl bg-muted hover:bg-muted/70 text-xs py-2"><MessageSquare className="h-3 w-3" /> Message</button>
                <button className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl bg-muted hover:bg-muted/70 text-xs py-2"><Mail className="h-3 w-3" /> Email</button>
              </div>
            </Surface>
          </motion.div>
        ))}
