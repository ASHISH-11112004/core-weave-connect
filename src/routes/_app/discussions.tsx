...
function Page() {
  return (
    <div className="space-y-6">
      <PageHeader
        title="Discussions"
        subtitle="Open conversations across the company"
        action={
        ...
        <Surface className="p-4 lg:col-span-1 h-fit">
          ...
          <ul className="flex lg:block gap-1 overflow-x-auto lg:space-y-1 -mx-1 px-1 lg:mx-0 lg:px-0">
            {["general", "engineering", "design", "culture", "product", "random"].map((c, i) => (
              <li
                key={c}
                className={`shrink-0 flex items-center gap-2 px-3 py-2 rounded-xl text-sm cursor-pointer ${i === 0 ? "bg-primary/10 text-primary" : "hover:bg-muted"}`}
              >
