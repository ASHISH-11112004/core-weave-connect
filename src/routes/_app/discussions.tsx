        ...
        <Surface className="p-4 lg:col-span-1 h-fit">
          ...
          <ul className="space-y-1">
            {["general", "engineering", "design", "culture", "product", "random"].map((c, i) => (
              <li key={c} className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm cursor-pointer ${i === 0 ? "bg-primary/10 text-primary" : "hover:bg-muted"}`}>
        ...
        <div className="lg:col-span-3 space-y-3">
          {discussions.map((d, i) => (
            <motion.div key={d.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
              <Surface hover className="p-5 flex gap-4">
