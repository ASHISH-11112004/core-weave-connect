        ...
        <Surface className="lg:col-span-2 p-5">
          ...
          <div className="grid grid-cols-7 gap-1.5 text-xs">
            ...
            {days.map((n) => {
              ...
              return (
                <div key={n} className={`aspect-square rounded-xl flex items-center justify-center text-sm transition cursor-pointer ${valid ? "hover:bg-muted" : "opacity-30"} ${hasEvent ? "bg-primary/10 text-primary font-semibold ring-1 ring-primary/20" : ""}`}>
                  {valid ? n : ""}
                </div>
              );
            })}
