  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Company events, town halls, and team rituals"
        action={
      ...
      <div className="grid lg:grid-cols-3 gap-4 sm:gap-6">
        <Surface className="lg:col-span-2 p-4 sm:p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-1">
              {["Day", "Week", "Month"].map((v) => (
                <button
                  key={v}
                  className={`px-3 py-1.5 text-xs rounded-xl ${v === "Month" ? "bg-primary text-white" : "bg-muted text-muted-foreground"}`}
                >
                  {v}
                </button>
              ))}
          ...
          <div className="grid grid-cols-7 gap-1 sm:gap-1.5 text-xs">
            {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
              <div key={i} className="text-center text-muted-foreground py-2">
                {d}
              </div>
            ))}
            {days.map((n) => {
              ...
              return (
                <div
                  key={n}
                  className={`aspect-square rounded-lg sm:rounded-xl flex items-center justify-center text-[11px] sm:text-sm transition cursor-pointer ${valid ? "hover:bg-muted" : "opacity-30"} ${hasEvent ? "bg-primary/10 text-primary font-semibold ring-1 ring-primary/20" : ""}`}
                >
                  {valid ? n : ""}
                </div>
              );
        ...
        <Surface className="p-4 sm:p-5">
          <h3 className="font-semibold mb-4 flex items-center gap-2">
            <CalIcon className="h-4 w-4 text-primary" /> Upcoming
          </h3>
              ...
              >
                ...
                <div className="mt-2 flex items-center gap-3 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1">
                    <CalIcon className="h-3 w-3" /> {e.date}
                  </span>
                  <span className="flex items-center gap-1">
